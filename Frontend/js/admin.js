...
const LAMBDA_ADMIN_URL =
  "https://hdc2oly7gs5v3kse5ifs6kjto40hlegl.lambda-url.us-east-1.on.aws/";
const LIFF_ID = "2008305063-P2NpboxX";

// NOTE: admin UI will display reporter_faculty, reporter_major, reporter_email
// if they are provided by the backend (Lambda/DB). Currently the AWS
// backend may not persist these fields — update the Lambda handlers and
// database schema to store: reporter_faculty, reporter_major, reporter_email
// if you want them to appear reliably in the admin UI.
          ...
          <tr>
            ...
            <td data-label="ผู้แจ้ง">
              <div style="font-size:0.875rem;">
                ...
                ${item.reporter_faculty ? `<div><strong>คณะ:</strong> ${item.reporter_faculty}</div>` : ""}
                ${item.reporter_major ? `<div><strong>สาขา:</strong> ${item.reporter_major}</div>` : ""}
                ${item.reporter_email ? `<div><strong>Email:</strong> ${item.reporter_email}</div>` : ""}
              </div>
            </td>
...
// Helper: fetch admin items and return array for client-side filtering
async function fetchAdminItems(payload = { search_mode: "admin" }) {
  try {
    const response = await fetch(LAMBDA_SEARCH_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
function applyFilters() {
  ...
  (async () => {
    const allItems = await fetchAdminItems({ search_mode: "admin" });
    ...
    if (typeFilter && typeFilter !== "") {
      filtered = filtered.filter((it) => it.item_type === typeFilter);
    }

    // Category filter: if "อื่นๆ" selected, include items whose category
    // is not one of the known categories or is empty/undefined
    if (categoryFilter && categoryFilter !== "") {
      if (categoryFilter === "อื่นๆ") {
        filtered = filtered.filter(
          (it) => !knownCategories.includes((it.category || "").trim()),
        );
      } else {
        filtered = filtered.filter(
          (it) => (it.category || "").trim() === categoryFilter,
        );
      }
    }
    ...
    // Date filter (match exact date string if provided)
    if (dateFilter && dateFilter !== "") {
      filtered = filtered.filter((it) => (it.date || "") === dateFilter);
    }
